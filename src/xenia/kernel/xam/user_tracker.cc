...
void UserTracker::AddDefaultProperties(uint64_t xuid) {
  ...
  Property PUID =
      Property(X_PROPERTY_GAMER_PUID,
               static_cast<int64_t>(user->account_info_.xuid_online));
  Property GAMER_HOST_NAME = Property(X_PROPERTY_GAMER_HOSTNAME, gamertag);
  Property GAMER_NAME = Property(X_PROPERTY_GAMERNAME, gamertag);
  Property GAMER_ZONE = Property(
      X_PROPERTY_GAMER_ZONE,
      static_cast<int32_t>(GAMERCARD_ZONE_OPTIONS::GAMERCARD_ZONE_PRO));
  Property GAMER_COUNTRY =
      Property(X_PROPERTY_GAMER_COUNTRY, cvars::user_country);
  Property GAMER_LANGUAGE =
      Property(X_PROPERTY_GAMER_LANGUAGE, cvars::user_language);
  Property PLATFORM_TYPE = Property(
      X_PROPERTY_PLATFORM_TYPE, static_cast<int32_t>(PLATFORM_TYPE::Xbox360));
  Property GAMER_MU = Property(X_PROPERTY_GAMER_MU, 0.0);
  Property GAMER_SIGMA = Property(X_PROPERTY_GAMER_SIGMA, 0.0);
...
void UserTracker::AddDefaultContexts(uint64_t xuid) {
  Property GAME_MODE = Property(X_CONTEXT_GAME_MODE, static_cast<uint32_t>(0));
  Property GAME_TYPE = Property(X_CONTEXT_GAME_TYPE, static_cast<uint32_t>(0));
  ...
  if (kernel_state()->emulator()->game_info_database()->HasXLast()) {
    ...
    std::optional<uint32_t> game_type_default =
        xlast->GetContextsQuery()->GetContextDefaultValue(X_CONTEXT_GAME_TYPE);

    if (game_mode_default.has_value()) {
      GAME_MODE = Property(X_CONTEXT_GAME_MODE, game_mode_default.value());
    }

    if (game_type_default.has_value()) {
      GAME_TYPE = Property(X_CONTEXT_GAME_TYPE, game_type_default.value());
    }
  }
...
std::u16string UserTracker::GetContextLocalizedString(uint64_t xuid,
  ...
  if (id == X_CONTEXT_GAME_MODE) {
    return GetContextGameModeLocalizedString(xuid);
  }

  if (id == X_CONTEXT_PRESENCE) {
    auto user = kernel_state()->xam_state()->GetUserProfile(xuid);
    if (!user) {
      return u"";
    }
...
std::u16string UserTracker::GetContextGameModeLocalizedString(
    uint64_t xuid) const {
  const Property* context = GetProperty(xuid, X_CONTEXT_GAME_MODE);
